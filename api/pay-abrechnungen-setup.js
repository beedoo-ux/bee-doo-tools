export default async function handler(req, res) {
  ...
  const sql = async (label, query) => {
    try {
      ...
      results.push({ label, status: r.status, ok: r.ok, res: text.slice(0, 300) });
      return r.ok;
  ...
  await sql('create_abrechnungen', `CREATE TABLE pay_abrechnungen (
  ...
  await sql('create_positionen', `CREATE TABLE pay_abrechnungen_positionen (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,

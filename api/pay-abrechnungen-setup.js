export default async function handler(req, res) {
  ...
  // ── 1. Abrechnungen Master ───
  await sql('create_abrechnungen', `CREATE TABLE IF NOT EXISTS pay_abrechnungen (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    periode TEXT NOT NULL,
    mitarbeiter_name TEXT NOT NULL,
    mitarbeiter_id TEXT,
    beschaeftigungsart TEXT DEFAULT 'hgb' CHECK(beschaeftigungsart IN ('hgb','angestellt')),
    status TEXT DEFAULT 'entwurf' CHECK(status IN ('entwurf','probeabrechnung','freigegeben','ausgezahlt')),
  ...
  await sql('idx_abr', `CREATE INDEX IF NOT EXISTS idx_pay_abr_periode ON pay_abrechnungen(periode, status);`);
  await sql('rls_abr', `ALTER TABLE pay_abrechnungen ENABLE ROW LEVEL SECURITY;`);
  await sql('pol_abr', `DROP POLICY IF EXISTS "pay_abr_anon" ON pay_abrechnungen;
    CREATE POLICY "pay_abr_anon" ON pay_abrechnungen FOR ALL TO anon USING (true) WITH CHECK (true);`);

  // ── 2. Abrechnungen Positionen ───
  await sql('create_positionen', `CREATE TABLE IF NOT EXISTS pay_abrechnungen_positionen (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    abrechnung_id UUID REFERENCES pay_abrechnungen(id) ON DELETE CASCADE,
    typ TEXT DEFAULT 'provision' CHECK(typ IN ('provision','eigenlead','empfehlung','speicher','einmalzahlung','abzug','sonstiges')),
    auftrag_nr TEXT,
    kunde TEXT,
    produkt TEXT,
  ...
  await sql('idx_pos', `CREATE INDEX IF NOT EXISTS idx_pay_pos_abr ON pay_abrechnungen_positionen(abrechnung_id, aktiv);`);
  await sql('rls_pos', `ALTER TABLE pay_abrechnungen_positionen ENABLE ROW LEVEL SECURITY;`);
  await sql('pol_pos', `DROP POLICY IF EXISTS "pay_pos_anon" ON pay_abrechnungen_positionen;
    CREATE POLICY "pay_pos_anon" ON pay_abrechnungen_positionen FOR ALL TO anon USING (true) WITH CHECK (true);`);
